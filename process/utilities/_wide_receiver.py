...
def create_wide_receiver(self, player_dict, index):
    ...
    strength = max(min(int(player_dict["strength"]), 85), 35)
    ...
    carrying = max(min(int(player_dict["carrying"]), 95), 45)
    self.set_player_integer_field('PCAR', index, carrying)
    
    catching = max(min(int(player_dict["catching"]), 99), 65)
    ...
    tackle = max(min(int(player_dict["tackle"]), 70), 15)
    ...
    injury = max(min(int(player_dict["injury"]), 99), 60)
