...
def create_middle_linebacker(self, player_dict, index):
    ...
    speed = max(min(int(player_dict["speed"]), 95), 65)
    self.set_player_integer_field('PSPD', index, speed)
    
    strength = max(min(int(player_dict["strength"]), 99), 60)
    ...
    acceleration = max(min(int(player_dict["acceleration"]), 95), 70)
    ...
    catching = max(min(int(player_dict["catching"]), 90), 20)
    ...
    kick_return = max(min(int(player_dict["kick_return"]), 65), 10)
    ...
    # importance, so use: max(min(ceil((([player_ovr]/100)^2) * 70) + ([player_ovr] - 70), 99), 15)
