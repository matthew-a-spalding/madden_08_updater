...
# TODO: Go through this code and make sure that we are assigning class, function, and variable 
# names according to the generally accepted Pythonic convention; be consistent in the use of 
# camelCase, Hungarian notation, and/or underscore_joined_names.
...
# This function is used in find_FBG_ratings_page, when the name on the FBGRatings.com roster was not an exact match with the name on NFL.com.
# It compares the positions of the two player pages to see if they are similar enough to be considered a match.
def positionsAreSimilar(position1, position2):
	...
	if position1.upper() == "RB" or position1.upper() == "HB" or position1.upper() == "FB" or position1.upper() == "WR" or position1.upper() == "TE":
		if position2.upper() == "RB" or position2.upper() == "HB" or position2.upper() == "FB" or position2.upper() == "WR" or position2.upper() == "TE":
			return True
		else:
			return False
	# If the first position is OL, T, OT, G, OG, LS, LT, LG, C, RG, or RT, position2 can be: OL, T, OT, G, OG, LS, LT, LG, C, RG, or RT.
	if position1.upper() == "OL" or position1.upper() == "T" or position1.upper() == "OT" or position1.upper() == "G" or position1.upper() == "OG" or position1.upper() == "LS" or position1.upper() == "LT" or position1.upper() == "LG" or position1.upper() == "C" or position1.upper() == "RG" or position1.upper() == "RT":
		if position2.upper() == "OL" or position2.upper() == "T" or position2.upper() == "OT" or position2.upper() == "G" or position2.upper() == "OG" or position2.upper() == "LS" or position2.upper() == "LT" or position2.upper() == "LG" or position2.upper() == "C" or position2.upper() == "RG" or position2.upper() == "RT":
			return True
		else:
			return False
	# If the first position is DL, DE, LE, or RE, position2 can be: DL, DE, LE, RE, DT, NT, LB, OLB, LOLB, ROLB, ILB, or MLB.
	if position1.upper() == "DL" or position1.upper() == "DE" or position1.upper() == "LE" or position1.upper() == "RE":
		if position2.upper() == "DL" or position2.upper() == "DE" or position2.upper() == "LE" or position2.upper() == "RE" or position2.upper() == "DT" or position2.upper() == "NT" or position2.upper() == "LB" or position2.upper() == "OLB" or position2.upper() == "LOLB" or position2.upper() == "ROLB" or position2.upper() == "ILB" or position2.upper() == "MLB":
			return True
		else:
			return False
	# If the first position is DT or NT, position2 can be: DL, DE, LE, RE, DT, or NT.
	if position1.upper() == "DT" or position1.upper() == "NT":
		if position2.upper() == "DL" or position2.upper() == "DE" or position2.upper() == "LE" or position2.upper() == "RE" or position2.upper() == "DT" or position2.upper() == "NT":
			return True
		else:
			return False
	# If the first position is LB, OLB, LOLB, ROLB, ILB, or MLB, position2 can be: DL, DE, LE, RE, LB, OLB, LOLB, ROLB, ILB, or MLB.
	if position1.upper() == "LB" or position1.upper() == "OLB" or position1.upper() == "LOLB" or position1.upper() == "ROLB" or position1.upper() == "ILB" or position1.upper() == "MLB":
		if position2.upper() == "DL" or position2.upper() == "DE" or position2.upper() == "LE" or position2.upper() == "RE" or position2.upper() == "LB" or position2.upper() == "OLB" or position2.upper() == "LOLB" or position2.upper() == "ROLB" or position2.upper() == "ILB" or position2.upper() == "MLB":
			return True
		else:
			return False
	# If the first position is DB, CB, S, SAF, FS, or SS, position2 can be: DB, CB, S, SAF, FS, or SS.
	if position1.upper() == "DB" or position1.upper() == "CB" or position1.upper() == "S" or position1.upper() == "SAF" or position1.upper() == "FS" or position1.upper() == "SS":
		if position2.upper() == "DB" or position2.upper() == "CB" or position2.upper() == "S" or position2.upper() == "SAF" or position2.upper() == "FS" or position2.upper() == "SS":
			return True
		else:
			return False
	# If either position is K or P and the other isn't one of those, the answer is no.
	if position1.upper() == "K" or position1.upper() == "P":
		if position2.upper() == "K" or position2.upper() == "P":
			return True
		else:
			return False
	logging.error("Unknown position1 passed to positionsAreSimilar:")
	logging.error("position1.upper() = %s" % position1.upper())
	logging.error("position2.upper() = %s" % position2.upper())
	return False

# This function takes in the two position strings for a player (found on NFL.com and FBGRatings.com)
# and uses them in determining which Madden position to assign this player. If no determination 
# can be made, but the input positions were in agreement, we assign "TBD" (to be determined). 
# If the positions are not in agreement, we assign "CONFLICT". The values of both "TBD" and 
# "CONFLICT" will require manual replacing after the output file has been fully generated.
def chooseBestPosition(position1, position2):
	# QB is easy.
	if position1.upper() == "QB":
		if position2.upper() == "QB":
			return "QB"
	...
	if position1.upper() == "RB":
		if position2.upper() == "RB" or position2.upper() == "WR" or position2.upper() == "TE":
			return "TBD"
	...
	if position1.upper() == "FB":
		if position2.upper() == "WR" or position2.upper() == "TE":
			return "TBD"
		if position2.upper() == "RB" or position2.upper() == "HB" or position2.upper() == "FB":
			return "FB"
		else:
			return "CONFLICT"
	# If position1 is WR, we are similar to RB, HB, FB, and TE, and can be specific with WR.
	if position1.upper() == "WR":
		if position2.upper() == "RB" or position2.upper() == "HB" or position2.upper() == "FB" or position2.upper() == "TE":
			return "TBD"
	...
	if position1.upper() == "TE":
		if position2.upper() == "RB" or position2.upper() == "HB" or position2.upper() == "FB" or position2.upper() == "WR":
			return "TBD"
	...
	if position1.upper() == "T" or position1.upper() == "G" or position1.upper() == "OT" or position1.upper() == "OG" or position1.upper() == "OL" or position1.upper() == "LS":
		if position2.upper() == "T" or position2.upper() == "G" or position2.upper() == "OT" or position2.upper() == "OG" or position2.upper() == "OL" or position2.upper() == "LS":
			return "TBD"
	...
	if position1.upper() == "LT":
		if position2.upper() == "LT" or position2.upper() == "T" or position2.upper() == "OT" or position2.upper() == "OL":
			return "LT"
		# If position2 is a different specific OL position, we have a TBD.
		elif position2.upper() == "G" or position2.upper() == "OG" or position2.upper() == "LS" or position2.upper() == "LG" or position2.upper() == "C" or position2.upper() == "RG" or position2.upper() == "RT":
			return "TBD"
		else:
			return "CONFLICT"
	# If position1 is LG, just make sure position2 is some sort of generic OL or G.
	if position1.upper() == "LG":
		if position2.upper() == "LG" or position2.upper() == "G" or position2.upper() == "OG" or position2.upper() == "OL":
			return "LG"
		# If position2 is a different specific OL position, we have a TBD.
		elif position2.upper() == "T" or position2.upper() == "OT" or position2.upper() == "LS" or position2.upper() == "LT" or position2.upper() == "C" or position2.upper() == "RG" or position2.upper() == "RT":
			return "TBD"
		else:
			return "CONFLICT"
	# If position1 is C, just make sure position2 is some sort of generic OL or C.
	if position1.upper() == "C":
		if position2.upper() == "C" or position2.upper() == "G" or position2.upper() == "OG" or position2.upper() == "OL":
			return "C"
		# If position2 is a different specific OL position, we have a TBD.
		elif position2.upper() == "T" or position2.upper() == "OT" or position2.upper() == "LS" or position2.upper() == "LT" or position2.upper() == "LG" or position2.upper() == "RG" or position2.upper() == "RT":
			return "TBD"
		else:
			return "CONFLICT"
	# If position1 is RG, just make sure position2 is some sort of generic OL or G.
	if position1.upper() == "RG":
		if position2.upper() == "RG" or position2.upper() == "G" or position2.upper() == "OG" or position2.upper() == "OL":
			return "RG"
		# If position2 is a different specific OL position, we have a TBD.
		elif position2.upper() == "T" or position2.upper() == "OT" or position2.upper() == "LS" or position2.upper() == "LT" or position2.upper() == "LG" or position2.upper() == "C" or position2.upper() == "RT":
			return "TBD"
		else:
			return "CONFLICT"
	# If position1 is RT, just make sure position2 is some sort of generic OL or T.
	if position1.upper() == "RT":
		if position2.upper() == "RT" or position2.upper() == "T" or position2.upper() == "OT" or position2.upper() == "OL":
			return "RT"
		# If position2 is a different specific OL position, we have a TBD.
		elif position2.upper() == "G" or position2.upper() == "OG" or position2.upper() == "LS" or position2.upper() == "LT" or position2.upper() == "LG" or position2.upper() == "C" or position2.upper() == "RG":
			return "TBD"
		else:
			return "CONFLICT"
	# If position1 is DL, try to go off of what we have in position2.
	if position1.upper() == "DL":
		if position2.upper() == "DL" or position2.upper() == "DE" or position2.upper() == "LB" or position2.upper() == "OLB"or position2.upper() == "LOLB" or position2.upper() == "ROLB" or position2.upper() == "ILB" or position2.upper() == "MLB":
			return "TBD"
	...
	if position1.upper() == "DE":
		if position2.upper() == "DL" or position2.upper() == "DE" or position2.upper() == "DT" or position2.upper() == "NT" or position2.upper() == "LB" or position2.upper() == "OLB" or position2.upper() == "LOLB" or position2.upper() == "ROLB" or position2.upper() == "ILB" or position2.upper() == "MLB":
			return "TBD"
		elif position2.upper() == "LE":
			return "LE"
		elif position2.upper() == "RE":
	...
	if position1.upper() == "LE":
		if position2.upper() == "RE" or position2.upper() == "DT" or position2.upper() == "NT" or position2.upper() == "LB" or position2.upper() == "OLB" or position2.upper() == "LOLB" or position2.upper() == "ROLB" or position2.upper() == "ILB" or position2.upper() == "MLB":
			return "TBD"
		elif position2.upper() == "DL" or position2.upper() == "DE" or position2.upper() == "LE":
			return "LE"
		else:
			return "CONFLICT"
	# If position1 is RE, most others will be a TBD, except for DL, DE, or RE.
	if position1.upper() == "RE":
		if position2.upper() == "LE" or position2.upper() == "DT" or position2.upper() == "NT" or position2.upper() == "LB" or position2.upper() == "OLB" or position2.upper() == "LOLB" or position2.upper() == "ROLB" or position2.upper() == "ILB" or position2.upper() == "MLB":
			return "TBD"
		elif position2.upper() == "DL" or position2.upper() == "DE" or position2.upper() == "RE":
			return "RE"
		else:
			return "CONFLICT"
	# If position1 is DT or NT, we can match DT, NT, or the generic DL.
	if position1.upper() == "DT" or position1.upper() == "NT":
		if position2.upper() == "DE" or position2.upper() == "LE" or position2.upper() == "RE":
			return "TBD"
		elif position2.upper() == "DL" or position2.upper() == "DT" or position2.upper() == "NT":
			return "DT"
		else:
			return "CONFLICT"
	# If position1 is LB, try to go off of position2.
	if position1.upper() == "LB":
		if position2.upper() == "DL" or position2.upper() == "DE" or position2.upper() == "LE" or position2.upper() == "RE" or position2.upper() == "LB" or position2.upper() == "OLB":
			return "TBD"
	...
	if position1.upper() == "OLB":
		if position2.upper() == "DL" or position2.upper() == "DE" or position2.upper() == "LE" or position2.upper() == "RE" or position2.upper() == "LB" or position2.upper() == "OLB" or position2.upper() == "ILB" or position2.upper() == "MLB":
			return "TBD"
		elif position2.upper() == "LOLB":
			return "LOLB"
		elif position2.upper() == "ROLB":
	...
	if position1.upper() == "LOLB":
		if position2.upper() == "DL" or position2.upper() == "DE" or position2.upper() == "LE" or position2.upper() == "RE" or position2.upper() == "ROLB" or position2.upper() == "ILB" or position2.upper() == "MLB":
			return "TBD"
		elif position2.upper() == "LB" or position2.upper() == "OLB" or position2.upper() == "LOLB":
			return "LOLB"
		else:
			return "CONFLICT"
	# If position1 is ROLB, we can match on LB, OLB, or ROLB.
	if position1.upper() == "ROLB":
		if position2.upper() == "DL" or position2.upper() == "DE" or position2.upper() == "LE" or position2.upper() == "RE" or position2.upper() == "LOLB" or position2.upper() == "ILB" or position2.upper() == "MLB":
			return "TBD"
		elif position2.upper() == "LB" or position2.upper() == "OLB" or position2.upper() == "ROLB":
			return "ROLB"
		else:
			return "CONFLICT"
	# If position1 is ILB or MLB, we can match on LB, ILB, or MLB.
	if position1.upper() == "ILB" or position1.upper() == "MLB":
		if position2.upper() == "DL" or position2.upper() == "DE" or position2.upper() == "LE" or position2.upper() == "RE" or position2.upper() == "OLB" or position2.upper() == "LOLB" or position2.upper() == "ROLB":
			return "TBD"
		elif position2.upper() == "LB" or position2.upper() == "ILB" or position2.upper() == "MLB":
			return "MLB"
		else:
			return "CONFLICT"
	# If position1 is DB, try to go off of what we have in position2.
	if position1.upper() == "DB":
		if position2.upper() == "DB" or position2.upper() == "S" or position2.upper() == "SAF":
			return "TBD"
	...
	if position1.upper() == "CB":
		if position2.upper() == "S" or position2.upper() == "SAF" or position2.upper() == "FS" or position2.upper() == "SS":
			return "TBD"
	...
	if position1.upper() == "S" or position1.upper() == "SAF":
		if position2.upper() == "DB" or position2.upper() == "CB" or position2.upper() == "S" or position2.upper() == "SAF":
			return "TBD"
		elif position2.upper() == "FS":
			return "FS"
		elif position2.upper() == "SS":
	...
	if position1.upper() == "FS":
		if position2.upper() == "CB" or position2.upper() == "SS":
			return "TBD"
		elif position2.upper() == "DB" or position2.upper() == "S" or position2.upper() == "SAF" or position2.upper() == "FS":
			return "FS"
		else:
			return "CONFLICT"
	...
	if position1.upper() == "SS":
		if position2.upper() == "CB" or position2.upper() == "FS":
			return "TBD"
		elif position2.upper() == "DB" or position2.upper() == "S" or position2.upper() == "SAF" or position2.upper() == "SS":
			return "SS"
		else:
			return "CONFLICT"
	# K is easy.
	...
	if position1.upper() == "P":
		if position2.upper() == "P":
			...
			return "CONFLICT"
	logging.error("Unknown position1 passed to chooseBestPosition:")
	logging.error("position1.upper() = %s" % position1.upper())
	logging.error("position2.upper() = %s" % position2.upper())
	return "CONFLICT"
...
class NFLandFBGSpider(CrawlSpider):
	...
	# Thus, trying to set them (as I originally did) in the __init__ by 
	# using: self.start_urls (etc.) will not work. Now, I'm doing it the 
	# proper way using [ClassName].[variable_name], as seen below.
	...
	def __init__(self, **kwargs):
		...
		# Initialize the list we will populate with all the NFL.com team roster page URLs.
		nfl_roster_urls = []
		
		# Enumerate the items (dictionaries) in the list of NFL team dicts.
		for index, dictTeamInfo in enumerate(settings.NFL_TEAMS):
			# Put a new copy of the base URL template for this team into the list.
		...
		NFLandFBGSpider.start_urls = nfl_roster_urls # eg. ["http://www.nfl.com/teams/philadelphiaeagles/roster?team=phi"]
		
		# Set our allowed domains to make sure we can visit pages on both NFL.com and FBGRatings.com.
		NFLandFBGSpider.allowed_domains = ["nfl.com", "fbgratings.com"]
		
		# Pass in the filtering regex using allow=() and/or restrict_xpaths=() to get the links for each player profile page, 
		# per http://doc.scrapy.org/en/latest/topics/link-extractors.html#module-scrapy.contrib.linkextractors.lxmlhtml
		NFLandFBGSpider.rules = [Rule(LinkExtractor(allow=(settings.NFL_PROFILE_LINKS_REGEX, )), callback="parse_NFL_profile_page")]
		#logging.warning("NFLandFBGSpider initialized with start_urls = %s, \n allow_rule = %s" % (nfl_roster_urls, settings.NFL_PROFILE_LINKS_REGEX))
		
		# We've now initialized the spider class with our values, so the only thing left to do is call the parent constructor.
	...
	def parse_NFL_profile_page(self, response):
		...
		# We're inside a new player profile page, so create a new item for this player.
		item = NFLPlayer()
		
	# CHECK AND UPDATE THIS WHOLE SECTION EACH YEAR AS NECESSARY.
	# Make sure that the xpath expressions below can be used to find the expected text in the source of any player profile page.
		
		# Start by populating the team field.
		item["team"] = response.xpath("//div[@class=\"article-decorator\"]/h1/a/text()").extract()[0] # eg. "Indianapolis Colts"
		
		# Next up is the player's first and last name.
		full_name = response.xpath("//meta[@id=\"playerName\"]/@content").extract()[0].split(None, 1) # eg. ["Andrew", "Luck"]
	...
	# TODO: Check that the team names found above at NFL.com ALWAYS work in the FBGRatings URL template, then remove this line and fill in and indent the line below.
	...
	def find_FBG_ratings_page(self, response):
		...
		# If we can't find text in an anchor tag that matches the pattern, ...
		if not response.xpath("//a/text()").re(regexPattern):
			...
			if not response.xpath("//a/text()").re(regexPattern):
				...
				# Before we return the item (to the pipeline for writing to the file), make sure we get either a Madden position, TBD, or CONFLICT.
				item["position"] = chooseBestPosition(item["position"], item["position"])
				return item
			else:
				...
				if positionsAreSimilar(item["position"], response.xpath(position_xpath).extract()[0]):
	...
	# Just be aware that FBGRatings tends to use 0 when they don't know the number. So, check for non-zero ones only.
		...
		if player_position_node:
			...
			item["position"] = chooseBestPosition(item["position"], player_position_text)
