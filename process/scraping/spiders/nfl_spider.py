...
class NFLandFBGSpider(CrawlSpider):
    ...
    # TODO: Test putting a comma into the name fields for a player, and see if 
    # it gets escaped or whatnot. See how the "NFL and FBG.csv" file we 
    # generate in the PlayerPipeline gets read in later when there is a commma 
    # in a field. If the comma gets into the .csv as is, we will probably just 
    # need to make sure these fields don't get populated with any commas in 
    # them. Fields that are most likely to be affected include first_name, 
    # last_name, and college. 
        
        item["first_name"] = full_name[0] # eg. "Andrew"
        item["last_name"] = full_name[1] # eg. "Luck"
        
        # Find the player's jersey number and position. If jersey number is 
        # empty, there must still be a '#' for this split to work.
        ...
        # The [1:] below takes only the chars from position 1 to the end of the 
        # string (string char indices start with 0).
        item["jersey_number"] = number_and_position[0][1:] # eg. "12"
        item["position"] = number_and_position[1] # eg. "QB"
        
        # All of the remaining NFL.com info (height, weight, age, college, and 
        # experience) is found in the div with class="player-info". However, 
        # some players may be missing their birth / age info, which changes how 
        # we handle that div's contents. First, check to see if there are 7 or 
        # only 5 /p/strong tags in the player-info div.
        player_info_p_strong_tags = response.xpath(
                "//div[@class=\"player-info\"]/p/strong/text()"
                ).extract()
        #logging.warning("player_info_p_tags = %s" % player_info_p_tags)
        
        if len(player_info_p_strong_tags) == 7:
            # The text() inside the third <p> tag in the 
            # <div class="player-info"> tag, which contains the height, weight, 
            # and age, also has a lot of whitespace chars, some of which are 
            # grouped into the first string by themselves, which is why the 
            # first string with real content (the height) is found at 
            # ...extract()[1], not ...extract()[0].
            height_weight_age_strings = response.xpath(
                    "//div[@class=\"player-info\"]/p[3]/text()"
                    ).extract() # a list of 4 strings, 
            # eg. [u'\r\n\t\t\t\t\t', u': 6-4 \xa0 \r\n\t\t\t\t\t', 
            # u': 240 \xa0 \r\n\t [...] \t\t\t', 
            # u': 25\r\n\t\t\t\t\t\t\r\n\t\t\t\t\t\r\n\t\t\t\t']
            ...
            # Get the player's college. This currently yields "No College" if 
            # the NFL.com page doesn't have that info.
            college_strings = response.xpath(
                    "//div[@class=\"player-info\"]/p[5]/text()"
                    ).extract()[0].split(None, 1) # eg. [u":", u"Stanford"]
